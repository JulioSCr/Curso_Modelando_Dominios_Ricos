...
namespace PaymentContext.Domain.Handlers
{
        ...
        IHandler<CreatePayPalSubscriptionCommand>
    {
        ...
        public ICommandResult Handle(CreateBoletoSubscriptionCommand command)
        {
            ...
            var address = new Address(command.Street, command.Number, command.Neiggbothood, command.City, command.State, command.Country, command.ZipCode);
            
            var student = new Student(name, document, email);
            var subscription = new Subscription(DateTime.Now.AddMonths(1));
            var payment = new BoletoPayment(
                command.BarCode, command.BoletoNumber, command.PaidDate,
                command.ExpiredDate, command.Total, command.TotalPaid,
                command.Payer, document, address, email);
        ...
        public ICommandResult Handle(CreatePayPalSubscriptionCommand command)
        {
            ...
            var payment = new PayPalPayment(
                command.TransactionCode, command.PaidDate,
                command.ExpiredDate, command.Total, command.TotalPaid,
                command.Payer, document, address, email);
